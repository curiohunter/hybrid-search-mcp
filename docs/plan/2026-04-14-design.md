...
#### 12.5 다음 구현 항목 (현재 코드 기준)

1. 영향 wiki 계산 최적화
   - 현재는 전체 `generate_all_wiki_pages()` 후 영향 page만 선별
   - 다음 단계는 `wiki_dependencies` / `files` 조인으로 직접 영향 page를 찾고
     필요한 page만 재생성

2. git delta 예외 케이스 보강
   - initial commit, merge commit, detached HEAD, shallow clone fallback 검증
   - rename / delete / binary / ignored path 처리 테스트 추가

3. hook 안정화
   - post-commit 중복 실행 방지(lock file 또는 pid guard)
   - 장시간 인덱싱 시 이전 작업과 겹치지 않도록 보호

4. wiki 재생성 범위 세분화
   - 직접 영향 page와 wikilink 간접 영향 page를 분리
   - 직접 영향은 구조적 자동 재생성, 간접 영향은 stale mark만 유지

5. 테스트 확장
   - `reindex --git-delta` CLI 경로 테스트
   - 대형 프로젝트 시뮬레이션에서 subset scan이 전체 scan으로 fallback하지 않는지 검증
   - STALE.md 자동 정리 동작 검증
