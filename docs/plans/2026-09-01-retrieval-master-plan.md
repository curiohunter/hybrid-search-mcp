...
- **WS4c 실사 결과 — 앵커 기계의 절반이 이미 있다**: wiki_dependencies
  ...
  좀비 정리가 소실 케이스를 담당한다. 남은 증분은 ① 미사용
  `chunk_ids` 컬럼을 finalize의 verify_references 결과로 채워
  **섹션 수준** 앵커로 승격 ② 해시 불일치 + 인용 청크 소실 =
  unprovable 분류 ③ 검색 노출 시 미검증 배지(trust_meta). 별도 작업
  단위로 분리.
