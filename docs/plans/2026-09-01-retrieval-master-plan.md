...
### 구현 결과 — WS4a rationale 레인 (2026-09-01)

genesis 슬라이스 신설(8문항, 정답 위치 실측 검증) 기준 **3/8 → 7/8**.

- **노드 rationale**: 함수/메서드/클래스에서 긴 서술(내러티브 docstring
  ≥200자, 연속 주석 그룹 ≥120자)을 별도 `rationale` 청크로 방출.
  코드 청크는 사본을 유지(동거 원칙). 머지 패스에서 면제(삼켜지면
  레인의 존재 이유가 소멸 — 첫 테스트에서 실측).
- **모듈 rationale**: miss 3건의 정답이 전부 모듈 docstring·최상위 주석
  블록이었다 — 노드 순회가 못 보는 곳. 파일당 1개 모듈 rationale 청크
  추가(이 레포 107개).
- **부산물로 잡은 결함 2건**: ① gold 파일이 인덱싱돼 벤치 질의가
  자기와 매칭(non-leaky 위반) → `.hybrid-search-ignore` 신설.
  ② memory-intent 토큰의 맨명사 "메모리"/"기억"이 메모리 기능에 대한
  질문을 회상 질의로 오분류 — 슬롯 플랜 전환+conv 스플라이스까지
  뒤집는 비용. 동사 결합형으로 협소화(G3 실측, 테스트 갱신).
- 하네스에 `accept_paths` + `commit:<substring>` 문법 추가 — genesis
  답은 파일·커밋·설계문서 어디에 살아도 정답이다(전부 수동 검증 후
  등재. 맨 경로 `.git-history/commits`는 아무 커밋이나 크레딧하므로
  금지).
- **잔여 G3 = 정직한 miss**: 한국어 질의 ↔ 영어 서사 크로스링구얼
  갭. KO→EN 다리는 메모리 레인에만 있고 rationale(코드 레인)엔 없다
  — 후속 항목. valuein gold 18/25·멀티홉 5/6 무회귀.
- valuein은 재빌드 전이라 rationale 미적용 — why 주석이 풍부한
  프로젝트이므로 다음 재빌드(0원, 벡터 재사용) 시 자동 편입.

### 구현 결과 — WS4b/4c 부분 (2026-09-01)

- **WS4b 프롬프트 재규정 완료**: SYNTHESIS_INSTRUCTIONS에 재서술 금지
  규칙 9 추가 — "코드 주석으로 그대로 옮겨도 성립하는 문장이면
  재서술이다, 잘라라". 83% 오염 사고를 근거로 명시.
- **WS4b 코사인 상한 게이트는 데이터-선행으로 보류**: 임계값 없이
  게이트를 넣는 것은 이 레포의 캘리브레이션 규율(임계값은 gold에서
  도출) 위반. 재규정된 프롬프트로 합성이 축적된 뒤 insight/재서술
  라벨 쌍에서 임계를 도출해 게이트화한다.
- **WS4c 실사 결과 — 앵커 기계의 절반이 이미 있다**: wiki_dependencies
  테이블이 페이지→파일+컴파일시점 해시를 이미 추적(affected 감지)하고
  좀비 정리가 소실 케이스를, lookup-wiki의 [STALE] 마크가 낡음 표시를
  담당한다.
- **WS4c(a) 완료 — 죽은 인용의 가시화**: verify_references가 소실된
  file:line 참조를 조용히 제거해 **주장이 근거 없이 검증된 산문처럼
  남던 것**을 ⟨unverified — source reference lost⟩ 마커 치환으로 교체.
  EA-Graph의 "낡은 것은 스스로 알리지 않는다"에 대한 직접 대응이고,
  grep 가능해서 페이지별 미검증 주장 수를 감사할 수 있다.
- **WS4c(b) 설계 함정 기록 — chunk_id 앵커는 불가**: chunk_id가 바이트
  오프셋 해시라 파일 앞부분 수정만으로 뒤쪽 청크 id가 전부 바뀐다 —
  id 앵커는 내용 불변 청크를 "소실"로 오판한다(가짜 unprovable).
  섹션 수준 앵커는 **qualified_name 기반**이어야 하며, 미사용
  chunk_ids 컬럼을 그 용도로 재정의하는 마이그레이션이 (b)의 실제
  작업이다. 이득은 재합성 비용 절감(파일이 바뀌어도 인용 심볼이
  무사하면 unaffected)이므로 우선순위 P2.
...
## 8.5 WS6 — 중앙 인덱스 서비스 (2026-09-01 사용자 제안, 설계 스케치)

맥미니(24시간, Tailscale 자동 복구)가 인덱스를 서빙하고 맥북은
임베딩처럼 원격 호출한다 — 재빌드 1회, 어디서 개발하든 같은 인덱스.

- **금지된 구현**: `~/.hybrid-search` 네트워크 공유(NFS/SMB/동기화 툴).
  SQLite·tantivy·usearch 전부 임베디드/mmap이라 네트워크 FS에서 손상
  경로다.
- **맞는 구현**: 기존 MCP 서버의 서비스화(Tailscale 바인딩, HTTP
  트랜스포트). 쓰기는 서버 프로세스 독점(writer lock 재사용), 동기화
  백본은 git(push → 맥미니 pull → post-merge 델타, ecae799로 이미
  자동).
- **최대 보상은 재빌드 절약이 아니라 기계 간 통합 대화 메모리** —
  트랜스크립트가 기계 로컬이라 지금은 맥북 인덱스에 맥북 세션만
  있다. 중앙화하면 "맥미니에서 어제 뭐 하기로 했지"가 맥북에서
  검색된다(verified handoff 쐐기의 직접 확장).
- **관건: 미커밋 작업**. 중앙 인덱스는 서버 체크아웃만 안다. in-flight
  오버레이는 검색이 도는 기계의 트리를 보므로, 원격 검색은 클라이언트
  dirty 파일을 못 본다. 단계: v1 읽기 중앙화(한계 명시 수용) → v2
  요청에 클라이언트 오버레이 동봉 → 오프라인 폴백은 로컬 BM25-only
  (임베딩 fail-open 패턴 재사용).
- 우선순위: WS4c(b)·WS5와 독립. 착수 전 별도 설계 문서 필요.
