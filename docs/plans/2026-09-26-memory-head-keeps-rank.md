...
**상태**: **채택** (2026-09-26 측정, §7). 사전 등록 `c1ef91a` · `01fc24d`, 구현 `c5612ac`.
...
같은 사본(`snap-m`)·같은 질문 634개. 덤프 M(`main` 코드) → F(`c5612ac`) → M′ 를 한 시간대에.
판정자는 opus 서브에이전트, 로컬만, 배치 10, 1·2차는 다른 호출.

| | 결과 |
|---|---|
| 노이즈 (M ≠ M′) | 0 / 634 |
| 무승부 (목록 동일) | 133 (프로브 72 · Set A 20 · Set B 41 — 골드는 예상대로 전부 동일) |
| **기준 1** 순서 보정 | 11쌍 (1→3 10 · 2→3 1, 보충 없음) — **11/11 = 1.00**, same 0, 순서 일치 1.00 ✅ |
| **기준 2** 프로브 F · M · same | **392 · 22** · 87 (판정 501) — F/(F+M) 0.947 ✅ |
| 순서 일치율 (프로브) | 0.916 · 1차 단독 F 402 · M 32, 2차 단독 F 416 · M 30 |
| **기준 3** Set A · Set B | M = F (0.80·0.70·0.6238 / 0.122·0.0244·0.0303) ✅ |
| **기준 4** 코드축 top5·recall@10·memory@3 | M = F = 0.92·0.77·0.16 ✅ — 영향 3문항 중 떨어진 것 없음 |
| 자기회수 (보고만) | M 554 · F 554 / 573 (창 안에 드는 수는 같고 순위가 오른다) |
| `after_distilled` 변화 (보고만) | 0 / 634 |

**F 가 진 22건** (§23 추적기 그대로, 재검색 22/22 덤프와 일치): 첫 하락 `_merge_memory_results`
13 · 없음 9. 13건 모두 **코드 레인 1위가 head 항목**(그중 10건은 답 없는 qa 레코드)이었고, 옛
코드는 그것을 3위로 내리면서 자기 답이 2위 → 1위로 **우연히** 올라갔다. 수정은 그 1위를
유지한다 — 설계대로의 동작이고, 게이트(B)가 답 없는 레코드를 빼면 10건은 사라질 모양이다.

**판정: 채택.** 다음은 §6 — main 반영 뒤 B+fix vs M+fix.
