...
### 4.1 판정자 보정 — 순서 보정 (합성, 2026-09-26 감독 결정)

§3 대로 Set A · B 목록은 M 과 F 가 같아 기존 보정(골드 목록 차이)은 대상이 0 이다.
2026-09-26 보정(5/5)의 **승계는 하지 않는다.** 그 보정은 구성원이 다른 목록에서 잰 것이고,
이번 변경은 구성원은 같고 **순서만** 바꾼다(주로 1 → 3 복원). 판정자의 same 비율이 0.70
이었으니, 순서 차이에 둔감한 판정자면 F · M 둘 다 거의 못 이기고 기준 2 가 0 ≥ 0 으로 공짜
통과할 수 있다. "순서만 다른 목록을 가려내는 능력"은 잰 적이 없다.

**대체 — 정답이 확실한 합성 짝으로 순서 판별력을 잰다.**
- 원본: 이번 측정의 **M 덤프**에서 골드 61 질문의 목록.
- 대상: 렌더 기준(`seen_hit_rank`, 판정자가 보는 글) 첫 정답 항목이 **1위**인 질문 → 그 항목을
  3위로 옮긴 짝(나머지 순서 유지) — 이번 결함이 만드는 1 → 3 과 같은 모양. 첫 정답이
  **2위**인 질문은 2 → 3 짝. 목록이 3개 미만이면 제외.
- 짝을 M(원본) · G(변형) 자리에 넣어 러너 `--pair` 로 만든다 — X/Y 배정·시드 · 1·2차 ·
  배치 10 · 결합 규칙 모두 기존 그대로. 정답(문구 기준 선호)은 러너가 렌더에서 계산하며,
  정의상 원본이 앞선다.
- 기준: **대상 ≥ 5, 일치율 ≥ 0.80, 판정자 same 은 불일치** (계획 `2026-09-25` §3.1 그대로).
- 대상이 5 미만이면 프로브의 M 목록에서 **자기 답 레코드**(프로브의 원 레코드)가 1위인 질문으로
  같은 1 → 3 짝을 만들어 보충한다(보충했다면 보고에 명시). 보충 짝의 정답은 자기 답이 위인 쪽.
- 판정문 v1 · 렌더 · 모델 · 배치 · 차수는 바꾸지 않는다. 미달이면 판정문을 혼자 고치지 않고
  멈춰서 보고한다.

## 5. 채택 기준 (전부 충족)

1. 판정자 순서 보정 (§4.1): 대상 ≥ 5, 일치율 ≥ 0.80.
2. 프로브 판정 **F 승 ≥ M 승** (same · 무승부 · 노이즈 · 미판정 제외). 단, 판정 사례 중
   same 이 아닌 결정(F + M)이 **0 이면 "판정 불능 — 보류"** 로 적는다(채택 아님).
