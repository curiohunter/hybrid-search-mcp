...
- **노이즈** — M 과 M′ 의 목록이 다른 질문. 코드 변경 탓으로 돌릴 수 없으니 판정하지
  않고 수만 보고한다. (계획 초안에 없던 대조. 지금까지 스냅샷 측정은 spread 0 이었으니
  0 이 예상이고, 0 이 아니면 그 수가 곧 이 계측의 노이즈 바닥이다.)
...
- qa 레코드(`node_type=qa_log`, 노트 포함): `질문:` 본문 `# Q:` 줄부터 `- **query_type**`
  앞까지를 공백 정리해 **200자**, `답:` 은 `qa_shape.answer_excerpt` 를 공백 정리해
  **400자**. 답을 소유하지 않으면 `(없음 — 질문과 당시 검색 결과만 담긴 기록)`.
...
- 대상: 골드(Set A+B) 중 판정 사례이면서 **문구 기준의 선호가 갈리는** 질문.
  문구 기준 선호 = `reference_arm`: 각 목록의 첫 정답 순위(`any_of` 문구가 content 나
  snippet 에 처음 나오는 순위, 없으면 11)가 앞선 쪽. 같으면(둘 다 없음 포함) 선호 없음 →
  분모에서 뺀다(판정자가 그런 질문에 무엇을 골랐는지는 보고만).
