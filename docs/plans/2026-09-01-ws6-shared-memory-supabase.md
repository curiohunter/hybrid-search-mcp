# WS6 — 기계 간 공유 메모리: 사용자 소유 Postgres 백엔드
...
## 9. 배포 결정 (2026-09-01, 라운드1 이후)

S6·S11 반영으로 설계가 표준 Postgres+pgvector만 타깃하게 되면서
"Supabase vs 맥미니"는 코드 분기가 아니라 **connection string 차이**가
됐다. 결정:

- **이 사용자의 배포: 맥미니 Docker Postgres**(`postgres` + pgvector,
  Tailscale 주소). 근거: ① Ollama로 이미 검증된 동일 운영 패턴(24/7 +
  Tailscale 자동 복구) ② 대화 원문(코드·키·개인 맥락 포함)이 집 밖으로
  나가지 않음 ③ $0 ④ OSS 도그푸딩 — "우리도 self-hosted로 쓴다"가
  BYO-Postgres 스토리의 가장 강한 증명. 원격 접근도 Tailscale로 해결
  (임베딩과 동일).
- **A안 기각과의 구분**: 기각된 것은 *우리 검색엔진 바이너리*의
  서비스화(버전 스큐, 우리 코드가 서버가 되는 부담)다. 스톡 Postgres
  컨테이너는 버전 스큐가 없고 백업이 표준이며, 장애 시에도 로컬
  폴백(규칙 3)으로 현행 동작이라 "조용한 장애"의 무게가 다르다.
- Supabase는 같은 코드로 URL만 바꾸면 되는 대안으로 유지 — 맥미니를
  끄게 되는 날의 이주 경로. 문서 제목의 "Supabase"는 역사적 경위로
  남긴다(조사·심사가 그 전제로 진행됨).
