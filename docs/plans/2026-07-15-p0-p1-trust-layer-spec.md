...
**Status: IMPLEMENTED (config/handler smoke까지) — 2026-07-15** (신규 테스트 11)

구현 노트:
- `codex_plugin.py`: 매니페스트(`.codex-plugin/plugin.json`) + 레거시
  hooks.json/config.toml 병행 설치, `setup --codex`/`doctor --codex`.
- **동봉 검사는 config/handler smoke이지 설치 E2E가 아님** (라운드 1 정정):
  훅 설정 존재 / in-process Stop 핸들러 왕복(qa 검사 후 즉시 삭제) /
  양측 MCP 설정 키 / 공유 root 문자열 4종. qa 로깅 비활성 시 SKIP(PASS 아님).
  **증명하지 않는 것**: Codex의 실제 subprocess 훅 호출, 라이브 MCP
  handshake+search, 실제 Claude-write → Codex-read 회수 — 이건 CX-T2/T3
  릴리스 게이트(맥미니 clean install)에서.
- teardown은 user-scope 매니페스트만 제거 (프로젝트 파일 불변 — 기존 정책과 일치).
...
| CX-T5c | 첫 인덱싱 처리량 (**비차단 telemetry 기준선** — 라운드 3 판정) | **산술 정정 (2026-07-27)**: 이전 기재 "±11.6% 분산"은 range의 오라벨(평균편차는 ±5.8%)이며, 일관 기준(range/min)으로는 ms/file 22.5% vs ms/chunk 28.2%로 청크 단위가 더 안정적이라는 주장 성립 안 함. 단위 선택(청크)의 방향성은 유지하되 근거 미확립 — 3점·무base·반복 중 청크수 변동(805→807)으로는 선형성·회귀판정("+20%") 불가. 기준선 기록만 유지: ripgrep 128f/805c 18.2s · pydantic 408f/3226c 58.4s · valueinmath 1595f/11957c 277.3s. 회귀 게이트 승격은 base 측정 + 동일 코퍼스 고정 + ≥5회 반복 후 |
...
- **Recency fast path (2026-07-15 실사용 관찰)**: Codex에서 "가장 최근에 한
  일이 뭐지" 질문이 hybrid_search 3회 + wiki 2페이지 = 고토큰 다단 폴백으로
  이어짐. 원인: 최상급-최근성 질문은 topical 토큰이 없어 relevance-first
  retrieval이 답할 수 없는 질문 클래스 (5월 기록이 상위 매칭). 설계:
  memory-intent + 최상급 recency 신호("가장 최근", "마지막", "latest") 감지
  시 벡터/BM25를 건너뛰고 timestamp 역순 conv/qa top-K를 결정론적으로 반환
  하는 전용 경로 — 1콜 완결, 토큰 1/5 이하 목표. blast radius 통제를 위해
  이번 PR 범위에서 의도적으로 제외.
