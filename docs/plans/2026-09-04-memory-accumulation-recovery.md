...
| **게이트 후 · conv 포함** | 0.93 | **0.53** | **0.556** |
| 게이트 후 · conv 제외(천장) | 0.93 | 0.80 | 0.582 |

**MRR 격차가 0.139 → 0.026으로 닫혔다.** 원시 대화가 정답을 밀어내던 현상의
대부분은 잡담이었고, 그 39.2%를 인덱스에서 걷어내자 사라졌다.

**남은 것은 top3 격차(0.53 vs 0.80)다.** 살아남은 conv 청크는 이제 실질적인
턴이지만, 통합 노트보다 질문과 어휘가 가까워 상위를 차지한다. 이건 코퍼스
문제가 아니라 **랭킹 정책 문제**다 — "같은 관련도라면 증류된 노트가 원시 턴보다
앞선다"는 규칙이 필요한데, 원칙 없이 가중치를 만지지 않았다. 이제 이 벤치가
있으므로 다음 라운드에서 **측정하면서** 손댈 수 있다.
