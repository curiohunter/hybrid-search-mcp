...
Configuration & Project Management handles all settings and project registry logic for hybrid-search-mcp. `config.py` loads `~/.hybrid-search/config.toml` into frozen dataclasses (EmbeddingConfig, SearchConfig, IndexingConfig, WikiConfig, SynthesisConfig) with sensible defaults, while `project.py` maintains a global SQLite registry mapping project names to filesystem paths and deterministic SHA-256 IDs (`src/hybrid_search/config.py`, `src/hybrid_search/project.py`).

## Key Design Decisions

- **Frozen dataclasses for config**: All config sections are frozen (immutable) dataclasses, preventing accidental mutation during hot-reload and ensuring thread safety (`src/hybrid_search/config.py`)
- **Deterministic SHA-256 project IDs**: `hashlib.sha256(path.encode()).hexdigest()[:12]` produces stable, filesystem-safe directory names from project paths (`src/hybrid_search/project.py`)
- **Legacy embedding field compatibility**: `EmbeddingConfig` preserves old fields (model, ollama_model, device, onnx_threads) for backward compatibility even though the current backend is OpenAI-only (`src/hybrid_search/config.py`)
- **TOML over JSON/YAML**: TOML chosen for human-readable config with strong typing support, loaded via Python 3.11+ tomllib (`src/hybrid_search/config.py`)
- **UPSERT in registry**: `register()` uses INSERT OR REPLACE keyed on project name, allowing path updates without duplicates (`src/hybrid_search/project.py`)

## Data Flow

```
~/.hybrid-search/config.toml
  → tomllib.loads() → raw dict
    → HybridSearchConfig(embedding=, search=, indexing=, wiki=, synthesis=)
      → consumed by Embedder, Orchestrator, Pipeline, WikiStore

~/.hybrid-search/global/project_registry.db
  → ProjectRegistry(global_dir)
    → register(name, path) → SHA-256 ID
    → get_by_name(name) → ProjectInfo(id, name, path)
    → list_all() → [ProjectInfo, ...]
```

## Caveats

- Config file absence silently uses all defaults — no warning when config.toml is missing
- Project ID collision (SHA-256 truncated to 12 hex chars) is astronomically unlikely but theoretically possible
- Registry uses a simple SQLite DB without WAL — concurrent CLI processes writing to the registry could conflict

## Related Modules

- [[hybrid-search]] -- server and CLI consume config and registry
- [[embedder----openai-api-backend]] -- EmbeddingConfig controls model, batch size, backend
- [[search]] -- SearchConfig controls RRF k, default BM25 weight, query classification
