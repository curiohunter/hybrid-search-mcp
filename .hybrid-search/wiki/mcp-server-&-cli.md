...
hybrid-search-mcp 프로젝트는 두 가지 인터페이스를 제공한다: Claude Code가 stdio로 연결하는 **MCP Server** (`server.py`)와 git hook/관리 작업용 **CLI** (`cli.py`). 서버는 3개 핵심 도구(hybrid_search, trace_callers, trace_callees)만 노출하여 시스템 프롬프트 토큰을 최소화하고, 나머지 10개 관리 도구는 CLI 명령으로 이관하여 MCP 오버헤드 없이 실행한다. 서버는 매 도구 호출 시 config.toml의 mtime을 확인하여 hot-reload를 지원한다.

## Key Design Decisions

- **MCP 도구 3개로 슬림화**: 기존 13개 도구에서 검색/추적 3개만 MCP로 남기고 나머지는 CLI로 이관. MCP 도구 스키마가 매 대화 시스템 프롬프트에 로드되어 토큰을 소모하기 때문 (`src/hybrid_search/server.py:L64`)
- **_HotReloadableConfig mtime 감시**: config.toml의 `stat().st_mtime`을 매 도구 호출 시 비교하여, 변경 시 Config/Embedder/Pipeline/Orchestrator를 재생성. 서버 재시작 없이 설정 변경 반영 (`src/hybrid_search/server.py:L29`)
- **_state 딕셔너리 패턴**: 클로저 내부에서 mutable dict로 공유 리소스(config, embedder, pipeline, orchestrator)를 관리. embedding 설정이 변경된 경우에만 Embedder를 재초기화하여 불필요한 모델 로드 방지 (`src/hybrid_search/server.py:L64`)
- **_dispatch_tool match 패턴**: Python structural pattern matching으로 3개 도구를 라우팅. 각 핸들러는 `tools/` 디렉토리의 별도 모듈에 위치하여 관심사 분리 (`src/hybrid_search/server.py:L64`)
- **CLI _detect_project**: 등록된 프로젝트 목록을 순회하며 cwd가 하위 경로인지 확인. 미등록 프로젝트는 자동 등록 (`src/hybrid_search/cli.py:L27`)
- **reindex 후 자동 체인**: 리인덱싱 완료 후 call graph re-resolution, wiki sync, staleness 마킹, gap flag 순서로 자동 실행 (`src/hybrid_search/cli.py:L54`)
...
[Claude Code] --stdio--> [MCP Server (server.py)]
                              |
                              +-- _maybe_reload() --> config.toml mtime 확인
                              |
                              +-- _dispatch_tool()
                                   |-- "hybrid_search" --> tools/hybrid_search.py --> SearchOrchestrator
                                   |-- "trace_callers"  --> tools/trace.py --> StoreDB call_edges
                                   |-- "trace_callees"  --> tools/trace.py --> StoreDB call_edges

[git commit] --> post-commit hook --> CLI reindex
                                       |
                                       +-- delta reindex --> call graph re-resolve
                                       +-- stale wiki marking
                                       +-- wiki-gaps flag
```
...
- **config reload 실패 시 이전 설정 유지**: `check_reload()`에서 예외 발생 시 경고만 로그하고 이전 config를 계속 사용. 잘못된 TOML 파일이 있어도 서버는 죽지 않지만 설정 변경이 무시됨 (`src/hybrid_search/server.py:L29`)
- **_dispatch_tool 클로저의 orchestrator 캡처**: hot-reload 후 trace 핸들러가 사용하는 `orchestrator` 변수가 외부 스코프에서 캡처되어, 다음 reload 사이클까지 stale 인스턴스를 참조할 수 있음 (`src/hybrid_search/server.py:L64`)
- **_detect_project 선형 탐색**: 등록된 모든 프로젝트를 순회하며 경로 매칭. 프로젝트 수가 많아지면 성능 저하 가능 (`src/hybrid_search/cli.py:L27`)
- **wiki-gaps 파일 무한 증가**: `_write_gap_flag()`는 append 모드로 타임스탬프를 계속 추가. 정리 메커니즘 없음 (`src/hybrid_search/cli.py:L27`)
- **MCP 인증/권한 없음**: stdio로 연결된 모든 클라이언트가 제한 없이 3개 도구를 호출할 수 있음

## Related Modules

- [[hybrid-search]] -- server.py가 초기화하는 핵심 검색 오케스트레이터와 프로젝트 레지스트리
- [[hybrid-search-(isolated)]] -- 동일 파일들의 isolated 뷰
- [[index-(isolated)]] -- CLI reindex 명령이 호출하는 AST chunker와 scanner
