...
Search (Isolated) covers infrastructure glue code in the search and CLI layers that doesn't have incoming call graph edges — including server initialization, hot-reloadable config, project auto-detection, and wiki gap flagging. These are top-level orchestration functions that wire together the search pipeline, config system, and CLI entry points.

## Key Design Decisions

- **Deterministic project IDs via SHA-256**: Project paths are hashed to create stable, filesystem-safe directory names for per-project storage
- **Hot-reloadable config**: `_HotReloadableConfig` checks config.toml mtime on every tool call, re-initializing only the Embedder when embedding settings change
- **Match/case tool dispatch**: Python structural pattern matching routes MCP tool calls to handlers, keeping the server entry point minimal
- **CWD-based project auto-detection**: CLI commands automatically find the registered project matching the current working directory

## Data Flow

```
server.py startup
  → load_config() → ProjectRegistry → Embedder → Pipeline → Orchestrator
    → tool call arrives via stdio
      → _HotReloadableConfig.check_reload()
        → _dispatch_tool(name, args)
          → tools/handler → response

CLI entry
  → argparse → _detect_project(registry, cwd)
    → cmd_* handler → StoreDB/Pipeline/WikiStore operations
```

## Caveats

- Server eagerly initializes all resources (Embedder, Pipeline, Orchestrator) at startup, even if no tool calls arrive
- Project detection scans all registered projects linearly — O(N) with project count
- Closure-captured orchestrator reference in server may become stale if hot-reload replaces it mid-request

## Related Modules

- [[mcp-server-&-cli]] -- the MCP server and CLI interface layer
- [[configuration-&-project-management]] -- config loading and project registry
- [[search]] -- SearchOrchestrator that powers hybrid_search tool
