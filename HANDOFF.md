...
| 4개 Tool 핸들러 (compile_to_wiki, lookup_wiki, check_wiki_staleness, refresh_wiki_page) | `tools/wiki.py` | ~180 |
...
**총 코드**: ~6,100줄 (29개 파일) | **MCP 도구**: 13개 | **테스트**: 161개 (10개 파일)
...
### MPS 가속 벤치마크 (M3 Pro, 18GB)
- **CPU**: 229.3초 (41.7 chunks/s)
- **MPS**: 192.9초 (49.6 chunks/s) → **19% 가속**
- e5-small(33M)은 경량이라 가속 폭 제한적. Qwen3-0.6B에서 더 큰 효과 예상
...
- **임베딩 모델**: `intfloat/multilingual-e5-small` (sentence-transformers 백엔드)
  ...
  - **권장 설정**: `device = "cpu"` + `onnx_threads = 6` (e5-small은 경량이라 MPS 19% 가속 대비 발열/팬 소음 트레이드오프 불리. Qwen3-0.6B 전환 시 MPS 재검토)
  - **향후 최적화**: ONNX + INT8 arm64 quantization (예상 2.7-3.3x 가속, 발열 없음) — Phase 5 후보
...
| 항목 | 파일 | 설명 |
|------|------|------|
| ~~dict 반환 → exception~~ | `tools/trace.py` `_open_stores` | ~~dict 반환~~ → `TraceError` 예외 도입 ✅ |
| ~~fuzzy search 비결정성~~ | `tools/trace.py` `_resolve_start` | ~~첫 번째만~~ → 모든 매치 반환으로 수정 ✅ |
| _split_large_chunks start_line | `index/ast_chunker.py` | P3 — "rough char estimate" 근사치, 수용 가능 |

### ~~Phase 5: Reactive Wiki Layer~~ ✅ 완료

4개 MCP 도구 구현 완료. 서버는 저장 + 의존성 추적, Claude가 콘텐츠 작성.

### Phase 6 후보

| 항목 | 설명 |
|------|------|
| Wiki 시맨틱 검색 | wiki_pages 콘텐츠를 벡터 인덱싱하여 의미 기반 wiki 조회 |
| ONNX INT8 quantization | ARM64 전용 INT8 양자화로 CPU 임베딩 2.7-3.3x 가속 |
| MindVault BM25 대체 | Hybrid Search가 MindVault의 BM25를 대체, MindVault는 Graph/Wiki 전담 |
