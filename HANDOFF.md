...
## 즉시 해야 할 것 — Phase 8c/8d: 검수 + 전체 파이프라인

Phase 8a+8b 완료. `generate-wiki`로 코드 구조 wiki 자동 생성 + DB sync 검증됨.

### Phase 8c: 검수 자동화 (`verify-wiki` 강화)
- `verify-wiki`가 모듈 트리 대비 wiki 페이지 존재/staleness 확인 (기본 구현 완료)
- uncovered 파일 목록, dependency 추적 강화

### Phase 8d: 전체 파이프라인 (완전 자동)
- post-commit hook → delta reindex → call graph re-resolve → module tree 변경 감지 → stale 마킹
- `/bootstrap-wiki` 스킬이 `generate-wiki` CLI 호출 후 Claude Agent가 내용 보강
