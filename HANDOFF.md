...
## 즉시 해야 할 것 — Phase 8b: 리프→부모 상향식 Wiki 생성

Phase 8a 완료. `generate-wiki-plan`으로 모듈 트리 자동 생성 검증됨. 다음 단계는 이 모듈 트리를 기반으로 실제 wiki 페이지를 자동 생성하는 것.

### Phase 8b: 리프 → 부모 상향식 Wiki 생성
1. `wiki-plan.json`의 모듈 목록을 위상정렬 순서(리프 먼저)로 처리
2. 각 모듈의 파일/청크를 읽고 Claude Agent가 wiki 페이지 작성
3. 부모 모듈은 자식 wiki를 합성하여 아키텍처 개요 생성
4. `architecture.md`: 모든 모듈 wiki를 합성하여 최상위 개요

### Phase 8c: 검수 자동화 (`verify-wiki` 강화)
- `verify-wiki`가 모듈 트리 대비 wiki 페이지 존재/staleness 확인 (기본 구현 완료)
- uncovered 파일 목록, dependency 추적 강화
