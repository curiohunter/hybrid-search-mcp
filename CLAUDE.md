...
## 측정 주기 — 스케줄러가 아니라 에이전트가 돌린다

`benchmarks/cycle.py` 가 전 축을 재고 지난 주기와 비교해
`~/.hybrid-search/benchmarks/cycle/` 에 기록한다. **크론·launchd에 걸지 않는다** —
측정은 수천 번의 검색이고, 사용자가 일하는 시간에 돌면 같은 임베딩 백엔드를 두고
그 사람의 검색과 다툰다(실측: 벌크 중에 pre-fetch가 BM25 전용으로 떨어졌다).

**대신 세션 시작 훅이 에이전트에게 지시한다.** 마지막 측정이 7일을 넘으면
SessionStart 컨텍스트에 이렇게 뜬다:
...
**그 줄을 보면 이 세션 안에서 처리할 것.** 사용자가 답을 기다리는 동안 말고,
긴 작업 사이나 세션 끝의 빈 시간에 백그라운드로 돌린다. 회귀가 나오면 그게 다음 작업이다.

- 판정 대기 사례가 쌓였다고 뜨면 `python benchmarks/judge_displacements.py --report <경로>`.
  사람 라벨은 절대 덮어쓰지 않는다.
- 이 줄은 **이 레포에서만** 뜬다. 밸류인 같은 측정 대상 프로젝트에서는 침묵한다 —
  거기서 칠 수 없는 명령이다.
