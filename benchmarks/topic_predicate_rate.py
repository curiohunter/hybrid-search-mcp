...
ap.add_argument("--config", required=True); ap.add_argument("--project", required=True)
ap.add_argument("--pairs", type=int, default=200000); ap.add_argument("--out", required=True)
a = ap.parse_args()
cfg = load_config(Path(a.config)); reg = ProjectRegistry(cfg.global_dir)
...
for _ in range(a.pairs):
    ...
    if SS._same_topic_strict(items[i], items[j]):
        acc.append((i, j))
print(f"{a.project}: n={n} sampled={a.pairs} accepted={len(acc)} rate={len(acc)/a.pairs*1e4:.1f} per 10k")
