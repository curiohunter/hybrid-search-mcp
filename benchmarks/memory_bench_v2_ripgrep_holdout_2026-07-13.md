...
- Cases: U1–U6 synthetic team-decisions (probes U5/U6 in KOREAN → EN
  corpus), R1–R5 REAL changes verified in CHANGELOG/git history
...
- Real-history update: **3/5**. R1 is the one true stale-fact failure
...
- Adversarial: 2/3. ADV3 (Korean probe) never retrieved the exact
  English qa at all (cross-language retrieval gap); grouping never
  engaged (both_found 0/3 — zero grouping errors on this corpus).
...
exact_first: **2/3** — decomposed: exact found 2/3, both found 0/3, exact first given both 0/1, adjacent not retrieved 2/3
